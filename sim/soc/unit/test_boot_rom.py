...
async def verify_rom_dual_ports(dut, addr_a_word, expected_a, addr_b_word, expected_b):
    ...
    # Aplica endereços em bytes e aciona o Valid
    ...
    # Lê saídas apenas se o valor estiver bem definido (para debug mais claro)
    try:
        actual_a = int(dut.data_a_o.value)
        actual_b = int(dut.data_b_o.value)
    except ValueError:
        log_error(f"Sinal indefinido (X/U) detectado na saída da ROM!")
        assert False
...
async def test_boot_rom(dut):
    ...
    hex_path = os.environ.get("HEX_PATH_FOR_TEST", "sim/soc/sw/build/bootloader.hex")
    if not os.path.exists(hex_path) or os.path.getsize(hex_path) == 0:
        project_root = os.environ.get("PWD", os.getcwd())
        hex_path = os.path.join(project_root, "build/boot/bootloader.hex")
    
    if not os.path.exists(hex_path) or os.path.getsize(hex_path) == 0:
         assert False, f"❌ Arquivo HEX não encontrado ou vazio: {hex_path}"
