...
async def bus_write(dut, address, data):
    """Realiza uma escrita no barramento simulado aguardando o handshake (rdy_o)."""
    dut.addr_i.value = address
    dut.data_i.value = data
    dut.vld_i.value = 1
    dut.we_i.value  = 1
    
    # Aguarda o VHDL sinalizar que capturou/processou (rdy_o = 1)
    while True:
        await RisingEdge(dut.clk)
        await ReadOnly()
...
async def bus_read(dut, address):
    """Realiza uma leitura no barramento simulado aguardando o handshake (rdy_o)."""
    dut.addr_i.value = address
    dut.vld_i.value = 1
    dut.we_i.value  = 0
    
    # Aguarda o VHDL sinalizar que o dado está pronto (rdy_o = 1)
    while True:
        await RisingEdge(dut.clk)
        await ReadOnly()
        if int(dut.rdy_o.value) == 1:
            # Lê o valor de data_o no momento exato em que é válido
