...
from sim.core.single_cycle.include.test_utils import log_header, log_info, log_success, settle, sign_extend
...
async def test_store_path(dut):
    ...
    # Verifica se a escrita na memória foi ativada (qualquer valor maior que 0)
    we_value = int(dut.DMem_writeEnable_o.value)
    assert we_value > 0, f"DMem_writeEnable_o não foi ativado (Valor: {we_value})"
