...
## Arbitro de Barramento (Bus Arbiter)

Além do interconectador que roteia os sinais para os escravos corretos, o SoC possui um **árebitro de barramento** (`bus_arbiter.vhd`) responsável por gerenciar o acesso ao barramento compartilhado entre múltiplos mestres (como CPU e DMA), evitando conflitos quando dois ou mais mestres tentam utilizar o barramento simultaneamente.
...
- **IDLE**: Estado ocioso. Monitora as requisições dos mestres (`m0_vld_i` e `m1_vld_i`). 
  - Lógica de arbitragem: Se `m1_vld_i='1'` → `GRANT_M1` (DMA vence)
...
3. **WAIT_Mx** (Estado de segurança):
   - **O barramento está effectivamente "congelado" para novas transações**
...
1. **Mapeamento de Sinais do Árbitro** (linhas 375-403):
   ...
   - Árbrito's CPU read data (`m0_rdata_o`) → CPU's DMem read data (`s_cpu_dmem_rdata`)
   - Árbrito's CPU ready (`m0_rdy_o`) → CPU's DMem ready (`s_cpu_dmem_rdy`)
   ...
   - Árbrito's DMA read data (`m1_rdata_o`) → DMA's memory read data (`s_dma_m_rdata`)
   - Árbrito's DMA ready (`m1_rdy_o`) → DMA's memory ready (`s_dma_m_rdy`)


2. **Conexão do Árbitro ao Interconectador** (linhas 396-403 e 417-423):
   - Árbrito's slave address (`s_addr_o`) → interconectador dmem_addr_i
   - Árbrito's slave write data (`s_wdata_o`) → interconectador dmem_data_i
   - Árbrito's slave write enable (`s_we_o`) → interconectador dmem_we_i
   - Árbrito's slave valid (`s_vld_o`) → interconectador dmem_vld_i
   - Interconectador's DMem read data (`dmem_data_o`) → árbrito's slave read data (`s_rdata_i`)
   - Interconectador's DMem ready (`dmem_rdy_o`) → árbrito's slave ready (`s_rdy_i`)
...
1. **Para Acesso à Memória (CPU ou DMA)**:
   - Master (CPU ou DMA) coloca endereço/dados e afirma seu sinal válido
   - Árbitro concede acesso baseado na prioridade (DMA > CPU)
   - Árbitro roteia os sinais do master para o interconectador
   - Interconectador decodifica o endereço e roteia para o escravo correto (RAM, ROM, etc.)
   - Escravo processa e retorna sinal pronto
   - Pronto retorna através do interconectador → árbitro → master correspondente
   - Master baixa seu sinal válido após receber pronto
   - Árbitro detecta master baixando válido e retorna ao estado IDLE

2. **Para Acesso aos Periféricos**:
   - Mesmo fluxo acima, mas o interconectador roteia para periféricos (UART, GPIO, etc.) baseado no endereço
   - Periféricos têm latências variáveis, tornando o handshake ready/valid essencial
